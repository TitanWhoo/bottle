...
class LabelGenerator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.defect_probability = config.get(
            "defect_probability", 1.0
        )  # 添加缺陷的概率
        self.perspective_probability = config.get(
    ...
    def generate_label(
        ...
        try:
            # 根据字体粗细选择字体
            if self.font_weight == "bold" and os.path.exists(
                self.font_path.replace(".ttf", "Bold.ttf")
            ):
    ...
    def add_defects(
        self, image: Image.Image, char_positions: List
        ...
        # 选择需要破坏的字符
        num_defects = random.randint(
            1, max(1, len(char_positions) // 3)
        )  # 随机破坏 1~33% 的字符
        ...
        for char, bbox in char_positions:
            ...
            if (char, bbox) in defect_chars:
                ...
                if defect_type > 0.85:
                    cv2.rectangle(
                        img, (x1, y1), (x2, y2), (255, 255, 255), thickness=-1
                    )
                # 2. 模糊字符
                ...
                # 4. 部分缺失字符
                    ...
                    if (
                        ...
                        # 3. 左上角缺失
                            ...
                            corner_width = random.randint(
                                char_width // 4, char_width // 2
                            )
                            corner_height = random.randint(
                                char_height // 4, char_height // 2
                            )
                        ...
                        # 7. 随机椭圆形缺失
                            ...
                            # 创建一个与字符大小相同的掩码
                            mask = (
                                np.ones((char_height, char_width), dtype=np.uint8) * 255
                            )
                            # 随机椭圆中心
                            center_x = random.randint(
                                char_width // 4, 3 * char_width // 4
                            )
                            center_y = random.randint(
                                char_height // 4, 3 * char_height // 4
                            )
                            # 随机椭圆轴长
                        ...
                        # 10. 笔画缺失效果
                            ...
                            if contours:
                                # 随机选择要擦除的笔画数量
                                num_strokes_to_erase = random.randint(
                                    1, min(3, len(contours))
                                )
                                strokes_to_erase = random.sample(
                                    contours, num_strokes_to_erase
                                )
    ...
    def apply_perspective(self, image: Image.Image) -> Image.Image:
        ...
        warped = cv2.warpPerspective(
            rotated, M_persp, (w, h), borderMode=cv2.BORDER_REPLICATE
        )
    ...
    def generate_random_specification(self) -> str:
        ...
        if "ml:{}g" in template:
            ...
            if random.random() < 0.7:  # 70%的概率使用小数
                g_value = round(
                    random.uniform(0.1, 10.0), random.choice([1, 2])
                )  # 随机保留1或2位小数
            else:  # 30%的概率使用整数
    ...
    def generate_random_speed() -> str:
        ...
        if random.random() < 0.7:  # 70%的概率使用整数+小数
            ...
            speed2 = round(
                base_speed + random.uniform(0.1, 1.0), 1
            )  # 增加0.1-1.0之间的随机值
        else:  # 30%的概率使用两个整数
            speed1 = int(base_speed)
    ...
    def _generate_single_sample(self, i: int) -> Dict[str, Any]:
        """
        生成单个样本图像和标注数据
        ...
        label, char_positions = self.generate_label(
            label_text, size=(width, height), font_size=font_size, color=blue_color
        )
        ...
        if random.random() < self.defect_probability:
            label_defect, valid_text, valid_boxes = self.add_defects(
                label, char_positions
            )
        else:
            label_defect = label
        ...
        annotation = {
            ...
            "label_type": (
                ...
                else self._detect_label_type(label_text)
            ),  # 记录标签类型
        }
        ...
        with self.lock:
            ...
            if (
                self.progress_counter % 10 == 0
                or self.progress_counter == 1
                or self.progress_counter == self.num_samples
            ):
                print(
                    f"已生成 {self.progress_counter}/{self.num_samples} 个标签图像..."
                )
    ...
    def generate_samples(self) -> None:
        ...
        if self.label_type != "random":
            print(
                f"正在生成 {self.num_samples} 个 '{self.label_type}' 类型的标签图像..."
            )
        else:
            print(f"正在生成 {self.num_samples} 个随机类型的标签图像...")

        print(f"使用 {self.num_workers} 个工作线程并行处理...")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            # 提交所有任务
            future_to_index = {
                executor.submit(self._generate_single_sample, i): i
        ...
        # 保存 JSON 标注文件
        with open(
            os.path.join(self.output_dir, "annotations.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(self.dataset, f, ensure_ascii=False, indent=4)

        # 保存 label.txt 文件
        with open(
            os.path.join(self.output_dir, "label.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        print(f"已生成 {len(self.dataset)} 个标签图像和标注数据！")
        print(
            f"总耗时: {elapsed_time:.2f} 秒，平均每张图像 {elapsed_time/max(1, len(self.dataset)):.4f} 秒"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--font_path",
        type=str,
        default="Song-Light.ttf",
        help="字体文件路径 (默认: Song-Light.ttf)",
    )
    parser.add_argument(
        "--output_dir", type=str, default="output", help="输出目录 (默认: output)"
    )
    parser.add_argument(
        "--num_samples", type=int, default=100, help="生成样本数量 (默认: 100)"
    )
    parser.add_argument(
        "--min_font_size", type=int, default=32, help="最小字体大小 (默认: 32)"
    )
    parser.add_argument(
        "--max_font_size", type=int, default=45, help="最大字体大小 (默认: 45)"
    )
    parser.add_argument(
        "--min_width", type=int, default=400, help="最小图像宽度 (默认: 400)"
    )
    parser.add_argument(
        "--max_width", type=int, default=600, help="最大图像宽度 (默认: 600)"
    )
    parser.add_argument(
        "--min_height", type=int, default=100, help="最小图像高度 (默认: 100)"
    )
    parser.add_argument(
        "--max_height", type=int, default=150, help="最大图像高度 (默认: 150)"
    )
    parser.add_argument(
        "--font_weight",
        type=str,
